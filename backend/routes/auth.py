...
def register():
    ...
    return jsonify({"success": True, "message": "Account created successfully!", "token": token, "user": user.to_dict()}), 201
...
def login():
    ...
    token = create_access_token(identity=str(user.id), expires_delta=timedelta(days=7))
    return jsonify({"success": True, "message": f"Welcome back, {user.name}!", "token": token, "user": user.to_dict()})
...
def profile():
    ...
    avg_score = sum(i.overall_score for i in interviews) / len(interviews) if interviews else 0
